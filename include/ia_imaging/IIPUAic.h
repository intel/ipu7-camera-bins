...
public:
    ...
    */
    virtual ia_err reInit(const ia_binary_data *aiqb, const ia_cmc_t *cmc, uint32_t max_stats_width,
        uint32_t max_stats_height, uint32_t max_num_stats_in, const ia_mkn *mkn,
        const int32_t aic_id = -1) = 0;
    ...
    virtual ia_err config(const cca_aic_config &conf, const cca_aic_kernel_offset &offset,
                          cca_aic_terminal_config &termConfig, int32_t aicId = -1, const int32_t* statsBufTypeToTermId = nullptr) = 0;
    /*!
    ...
    virtual ia_err run(const cca_pal_input_params& params, cca_3a_plus_results& aaaResults,
                       cca_multi_pal_output& output, uint8_t bitmap, int32_t aicId = -1) = 0;
    ...
    * \return                           Error code.
    */
    virtual ia_err runAic(const cca_pal_input_params &params, cca_3a_plus_results &aaaResults, int32_t aicId = -1) = 0;
     ...
     * \return                   Error code for status. zero on success, non-zero on failure
     */
    virtual ia_err runKernels(uint32_t groupId, uint64_t frameId, cca_binary_data *output_data, uint32_t fragment_index = 0U, int32_t aicId = -1) = 0;

    virtual ia_err UpdateConfigurationResolutions(const cca_aic_config& conf, bool isKeyResolutionChanged, int32_t aicId) = 0;

    virtual cca_stats* queryStatsBuf(cca_stats_buf_status status, uint64_t frame_id = cca::INVALID_FRAME_ID) = 0;
