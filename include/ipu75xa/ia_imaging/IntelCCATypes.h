...
typedef struct
{
    bool have_manual_settings;            /* Whether to use accuarate r per g and b per g values to override actual awb results. */
...
typedef struct
{
    ...
    float32_t manual_gamma;                     /*!< Optional. manual gamma for GTM. */
...
typedef struct
{
    ...
    ia_rectangle statistics_crop_area;  /*!< RGBS and AF grid area crop with respect to full field of view of sensor output using (relative)ranges from ia_coordinate.h. */
    bool using_rgbs_for_aec;            /*!< use rgbs to generate the AE histogram */
    uint32_t reset_hist;                /*!< reset the histogram, operate on bit, refer to CCAHISTType for the bit correspondence.*/
...
/*!
*  \brief ia camera rectification parameters structure for image LDC...
*/
#ifdef PAC_ENABLE
typedef struct
{
    bool enabled;                  /*!< enable/disable the API */
    ia_rectify_parameters_t rectify_params; /*!< rectification params */
} cca_rectification;

/*!
*  \brief ia neural network tone mapping parameters...
*/
typedef struct
{
    float32_t* nn_coeff_out;     /**< Coefficients obtained from neural network inference */
    int32_t coefficient_width;   /**< Width of coefficients structure */
    int32_t coefficient_height;  /**< Height of coefficients structure */
    int32_t coefficient_depth;   /**< Depth of coefficients structure */
} cca_nntm_params;
#endif
...
#ifdef PAC_ENABLE
    ia_camera_parameters_t camera_params;      /*!< Optional. sensor/view intrinsic/extrinsic params */
    cca_rectification rectification;           /*!< Optional. rectification with intrinsic/ldc params */
    uint32_t fragment_index;                  /* !< Optional. indicates the current stripe index that we calculate parameters for.In case striping is not supported 0 should be used. */
    ia_nntm_parameters_t nntm_params;          /*!< Optional. Neural network tone mapping params */
#endif
...
typedef struct
{
    ...
    int32_t media_format_stills;
    tnr7us_trigger_info_t tnr7us_trigger_info;
    seg_net_info_t seg_net_info;
} cca_cmc;
...
typedef enum {
    MultiCCA_Unknown = -1,  /*Unintialized Mode*/
...
typedef struct
{
    ...
    cca_multi_mode group_type; /*!< 0, X:X 1, 1:X. */
    cca_multi_id multi_id[MAX_CAM_NUM];
    cca_init_params* init_params[MAX_CAM_NUM];
}cca_multi_cam;
